...
GLuint Shader::loadShader(GLenum shaderType, const std::string &shaderSource) {
    ...
    if (shader) {
        ...
        if (!shaderCompiled) {
            GLint infoLength = 0;
            glGetShaderiv(shader, GL_COMPILE_STATUS, &infoLength);
...
void Shader::drawModel(const Model &model) const {
    ...
    const auto* texturePtr = model.getTexturePtr();
    glBindTexture(GL_TEXTURE_2D, texturePtr ? texturePtr->getTextureID() : 0);

    // Draw as indexed triangles
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(model.getIndexCount()), GL_UNSIGNED_SHORT, model.getIndexData());
